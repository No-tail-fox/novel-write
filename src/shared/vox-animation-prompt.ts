...
import { shotcraftRecipe } from './shotcraft-recipes';
export function voxAnimationMessages(input:VoxGenerateRequest) {
  return [
    ...
    {role:'user' as const,content:JSON.stringify({request:input.prompt,referenceTemplate:voxTemplate(input.templateId)?.name,shotRecipe:shotcraftRecipe(input.templateId),props:input.props,durationMs:input.durationMs,ratio:input.ratio,currentCode:input.source||undefined})},
  ];
}
