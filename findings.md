...
# 四工作台生产闭环完善发现（2026-08-24）

- 用户明确要求 HTML 视频、AI 漫剧、VOX 视频、音乐 MV 都尽可能完善；不能只把智能成片作为唯一成熟主链路。
- 本轮以“创建、恢复、编辑、局部返工、预览、导出、失败恢复”作为统一成熟度量尺。
- 现有仓库已经把 VOX、AI 漫剧和 HTML 视频写入统一 Task 表；音乐 MV 也走普通任务运行器，因此应扩展现有任务系统，不新建领域孤岛。
- 当前工作树只有用户既有未跟踪目录和输出图片，本轮不清理、不覆盖。
- 音乐 MV 页面当前只有一次性创建表单；创建后调用 `openTaskDetail()` 进入普通任务详情，没有恢复既有 MV 参数、歌词结构和领域返工的工作区。
- 音乐 MV 底层并非空壳：运行器会探测本地音频时长、构建歌词段落与镜头、跳过普通旁白、生成图片并写入专用剪映 sidecar 草稿；短板主要在创建后的编辑与返工 UI/合同。
- HTML 视频已支持既有场景的文案/版式/字幕/提示词修改、素材替换/重生/抠图、逐场景配音重生、封面重画、源码编辑/检查和整片重渲染；当前 API 没有场景新增、删除、复制或重排命令。
- AI 漫剧共享模型已经导出 `appendMotionComicEpisode`、`appendMotionComicScene`、`appendMotionComicShot`，页面也有统一保存、图片生成、配音和本地成片能力；需要继续核对结构操作是否完整暴露，以及删除/重排/一致性校验是否形成用户闭环。
- 四条链路成熟度不能只按“页面数量”判断：应区分底层已有能力、UI 是否暴露、修改后是否正确失效旧产物、是否能局部重跑。
- AI 漫剧页面已经真实暴露新增分集、场景和镜头，并在 `motion-comic.test.ts` 覆盖追加后的模型合法性；目前没有删除、复制或重排函数，结构变化后的时间轴只能追加不能正规化。
- AI 漫剧验证器已检查分集/场景/镜头连续编号、引用归属、对白 cue、素材/job 引用和 ready 状态的一致性，可作为新增结构操作后的质量门禁。
- VOX 共享模型只有 starter 创建、解析和验证，没有任何节拍/镜头结构操作；页面只能修改现有四镜字段，因此“固定 30 秒四镜”是模型操作层的真实限制，不只是创建页文案。
- HTML 视频 `applyHtmlVideoSceneChanges()` 只编辑既有场景；结构扩展必须同步正规化 scene index，并重映射/裁剪 `assets`、`voices`、`compositions`，同时把预览与输出标记为待重建。
- 音乐 MV 适合采用创建/工作区双态：已有任务仍使用统一 Task 和通用 `TaskArtifactPreview` 的图片、字幕、配音、视频与重跑能力，领域页只需提供既有任务打开、MV 参数/歌词概览和正确的返工导航。
- Director Desk 共享组件已有新增集/场景/镜头入口，但没有移动和删除命令；已扩展为选中镜头的上移、下移和删除菜单，VOX 与漫剧复用同一交互。
- VOX 结构变更必须保持首个钩子不超过 3 秒；移动长节拍到首位会返回可见错误，避免保存出验证器必然拒绝的文档。
- HTML IPC 有严格的一通道一 schema 库存测试；新增场景结构命令必须同步 `types`、`ipc-contract`、preload、browser fallback、StoryDreamApi 和 main handler。
- HTML 场景结构变化选择从 `assets` 阶段失效：保留场景文案/规划，清空可能错位的图片、配音、预览与成片；这是比路径重命名更可靠的本地恢复策略。
- 音乐 MV 已新增受治理的完整参数快照更新：数据库原子写入歌词、节奏、字幕、视觉、音频、模板与画幅，artifact state 从第 0 步失效，再复用既有任务运行器和逐镜返工。
- 音乐 MV 页面现在有创建/已有任务双态；运行中任务只读，已完成/失败/暂停任务可回填领域参数并“保存并重新生成”，也可进入通用任务详情处理图片、字幕和草稿。
- VOX 创建合同已扩展为 30 秒/4 节拍、45 秒/6 节拍、60 秒/8 节拍；starter 会按目标数量重新切分源文并分配连续时间，不追加空白镜头。
- Director Desk 删除镜头使用共享确认 Dialog，取消不修改草稿，确认后才进入 VOX/漫剧领域删除函数。
- `App.tsx` 原来静态导入 53 KB 的 browser fallback 及其依赖，导致入口在新增命令后超过预算；惰性代理让 Electron 启动不再加载任何 fallback 实现，浏览器预览首次 API 调用才加载独立 chunk。
- 浏览器 QA 在 1440x900 与 1040x720 覆盖 VOX 创建结构、音乐 MV 创建页、HTML 场景规划、AI 漫剧创建页和 VOX Director Desk 删除确认；所有状态横向溢出为 0，固定控件裁切为 0，route error 与 console error/warning 为 0。
- 视觉复审发现并修正 VOX 结构摘要与结构预览不同步；45 秒预设现在显示 6 个节拍及 3s + 5x8.4s 的可见分配。
