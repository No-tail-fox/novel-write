...
- 当前 `Stitch MCP` 已通过 API Key 完成 `initialize`、`tools/list` 和只读 `list_projects` 验证。
- Stitch 暴露 15 个工具；生成链路涉及 `create_project`、`generate_screen_from_text`、`get_screen` 和 `list_screens`。
- 用户已有 12 个私有项目；本次必须创建独立测试项目，避免污染或修改既有项目。
- 测试设计选择与当前仓库领域一致的桌面端小说写作工作台，便于直接判断生成结果是否有实际价值。
- 已核对准确 schema：`create_project` 只需可选标题；生成必需 `projectId` 与 `prompt`，可指定 `DESKTOP` 和 `GEMINI_3_1_PRO`。
- 生成工具明确要求耗时/超时时不得重试；后续只能通过 `list_screens` 和 `get_screen` 轮询。
- 已创建独立私有测试项目：`projects/1675698795742670867`，标题为 `Codex Stitch Test - Novel Writing Workspace - 2026-08-02`。
- 唯一一次生成请求在客户端等待上限后超时；Stitch 文档明确说明连接超时时生成仍可能成功，禁止重试，应轮询屏幕读取接口。
- 轮询 1：`list_screens` 返回空对象，项目暂未出现可读取屏幕，继续等待后台生成。
- 轮询 2：仍返回空对象；Stitch 后台任务尚未暴露屏幕资源。
- 轮询 3：仍返回空对象；通过独立 MCP HTTP 请求确认认证和 `list_screens` 调用均为 HTTP 200。
- 轮询 4：仍返回空对象；生成结果尚未进入项目屏幕列表。
- 轮询 5：仍返回空对象；生成结果尚未进入项目屏幕列表。
- 轮询 6：仍返回空对象；生成结果尚未进入项目屏幕列表。
- 轮询 7：仍返回空对象；生成结果尚未进入项目屏幕列表。
- 轮询 8：仍返回空对象；生成结果尚未进入项目屏幕列表。
- 轮询 9：仍返回空对象；生成结果尚未进入项目屏幕列表。
- 轮询 10：仍返回空对象；本轮 10 次轮询结束，未出现屏幕资源。
- 结论：Stitch MCP 接入链路本身可用，但本次无法证明生成任务最终产出设计图。由于协议要求超时后不得重复提交，后续只能在 Stitch 后台出现屏幕后再用 `list_screens`/`get_screen` 读取，或由用户在 Stitch 网页端确认任务状态。
...
- 全局结构是稳定的“任务/步骤侧栏 + 六标签工作区”，工作区内部自行滚动，标签栏位置不随内容变化。
- 文案页不是只读结果：顶部是最终成稿，下面逐场景显示正文、标题、短字幕、背景词、前景词和画面预设。
- 素材页按场景组织真实缩略图；背景、前景与“手动添加前景”并列，悬停出现预览、重生成、编辑/替换等动作。
- 配音页先选供应商、音色和语速，每个场景都有原生音频播放器、时长、对应文案和“重配”。
- 动画预览页是主工作面：左侧 9:16 播放器，右侧场景列表；场景卡可切换 17 类预设、隐藏背景/标题，并支持选中/播放。
- 封面页是带真实封面缩略图的参数面板，模板、构图方向、标题文字和比例用紧凑选项组编辑。
- 出片页用真实 video 播放成品，路径和“打开成片 / 调整背景音乐 / 重新出片”紧邻结果。
- 视觉优势来自紧凑的信息层级、真实媒体占据主视觉、低圆角弱边框、青绿色单一功能强调，以及每个阶段都能立即验证结果；不是靠装饰背景。

## 当前判断

- 现有项目底层已经有 scene change、asset replace/regenerate、composition、voice、cover、output 等数据能力，主要缺口集中在页面编排、动作暴露和媒体预览完整性。
- 不能继续把未生成状态渲染成大面积占位卡；应保留编辑器并对缺失媒体给出明确的阶段内操作。

## 实现与验收结论

- 六阶段采用统一的任务侧栏、固定标签栏和工作区内滚动；切换标签、任务或自动/编排模式时媒体面板回到顶部，避免用户误以为内容缺失。
- 生图质量覆盖是任务级可选参数：`null` 表示不启用，启用后只向 OpenAI 兼容图片接口传 `low/medium/high` 的 `quality`；供应商、模型、比例、分辨率和并发继续使用既有配置。
- HTTP 402 是远程图片供应商响应，不是本地提示词或质量参数校验错误；质量只能影响后续消耗，不能解决账户已经无余额的请求。
- 可视编排不能直接把任务 HTML 作为 `storydream-media://` 加载后继续引用 `file://`，Chromium 会阻止跨方案子资源；协议响应必须在安全校验后结构化重写脚本、图片和音频地址。
- `srcdoc` 虽能规避子资源问题，但与 HyperFrames 所需沙箱权限组合会触发 Chromium 安全警告；独立协议 iframe 加运行时消息是更稳妥的边界。
- `storydream-media://` iframe 与 `file://` 应用页面跨源，父页面读取 `contentDocument` 返回空属于预期同源保护。背景图验收改为由组合运行时在图片加载/失败后上报 `naturalWidth/naturalHeight` 结论和实际 `[src]` 列表。
- 最终 Electron 报告确认背景图已加载，四个组合媒体引用全部使用 `storydream-media://`，HyperFrames 检查为 0 错误，T0/T1/T20/T21 四轨和六步骤完整，运行时错误为 0。
- 最新桌面与 1080x720 紧凑截图中，封面说明为中文，出片首屏完整，错误感叹号水平/垂直居中，页面无横向溢出和可见控件裁切。
