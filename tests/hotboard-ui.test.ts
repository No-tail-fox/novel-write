...
describe('hot board workbench ui', () => {
  it('owns refresh, filters, source assessment, external links, and creation handoff', async () => {
    ...
    expect(page).toContain("import { Button, Dialog, IconButton } from '../../ui'");
    expect(page).toContain('热点正文');
    expect(page).toContain("'正文'");
    expect(page).toContain('openSourceDialog(item)');
    expect(page).toContain('retrySourceContent');
    ...
    expect(page).toContain('没有找到可用的联网结果');
    expect(page).not.toContain('<Tooltip content="打开原文">');
    expect(page).toContain("source?.kind === 'page' ? '页面正文' : '来源摘要'");
    expect(page).toContain("source?.kind === 'page' ? '图文正文' : '图文摘要'");
    expect(page).toContain('mediaCount > 0 ? visualContentLabel : contentLabel');
    expect(page).toContain('hot-board-source-reader');
    expect(page).toContain('HotBoardMediaGallery');
    expect(page).toContain('hot-board-reader-gallery');
    expect(page).toContain('data-hot-board-media-count');
    expect(page).toContain('referrerPolicy="no-referrer"');
    expect(page).toContain('loading="lazy"');
    expect(page).toContain('查看图片');
    expect(page).toContain('图片加载失败');
    expect(page).not.toContain('hot-board-source-content');
    expect(page).not.toContain('hot-board-content-placeholder');
    expect(page).not.toContain('查看正文');
    ...
    expect(css).toContain('62px 172px');
    expect(css).toContain('white-space: nowrap');
    expect(css).toContain('.hot-board-create-action');
    expect(css).toContain('.hot-board-reader-content');
    expect(css).toContain('.hot-board-reader-gallery');
    expect(css).toContain('.hot-board-image-viewer');
