...
describe('real Electron smoke contract', () => {
  ...
  it('exercises preview-only caption controls and preview-stage invalidation in real Electron QA', async () => {
    ...
    expect(qa).toContain("'[data-html-video-edit-field=\"captionLayout\"] [role=\"group\"][aria-label=\"字幕区域\"]'");
    expect(qa).toContain("'input[aria-label=\"强调十六进制颜色\"]'");
    expect(qa).toContain("item.textContent.includes('保存字幕')");
    expect(qa).toContain('if (captionUpdate.completedStepCount !== 5)');
    ...
    expect(qa).toContain('cueSamples.some((sample) => sample.visibleCount !== 1)');
    expect(qa).toContain('state.captionLayout?.widthRatio !== 0.76');
    expect(qa).toContain("'post-layout stable foreground frame'");
    expect(qa).toContain("'compact stable foreground frame'");
    expect(qa).toContain('effectsDesktop.previewPhoneSize.height < 400');
    expect(qa).toContain('effectsCompact.previewPhoneSize.height < 260');
